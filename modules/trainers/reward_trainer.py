...
from modules import RewardConfig
...
class RewardTrainer(Trainer):
    ...
    def __init__(
        ...
        self.args.bsr_lambda = self.args.bsr_lambda * self._train_batch_size

        # Locally log the reward trajectories
        self.stack_chosen_rewards = []
        self.stack_rejected_rewards = []
        self.stack_z_loss = []
    ...
    def compute_loss(
        ...
        ### Support diverse baselines
        if self.args.use_margin:
            loss = -nn.functional.logsigmoid(
                rewards_chosen - rewards_rejected - self.args.margin
            ).mean()
        elif self.args.use_hinge:
            loss = torch.relu(
                self.args.margin - (rewards_chosen - rewards_rejected)
            ).mean()
        elif self.args.use_normalization:
            reward_vector_norm = torch.sqrt(
                rewards_chosen ** 2 + rewards_rejected ** 2
            )
            loss = -nn.functional.logsigmoid(
                (rewards_chosen - rewards_rejected) / reward_vector_norm
            ).mean()
        else:
            loss = -nn.functional.logsigmoid(rewards_chosen - rewards_rejected)

        if self.args.use_eurus:
            loss += - torch.log(nn.functional.sigmoid(rewards_chosen)) - torch.log(nn.functional.sigmoid(-rewards_rejected))
            loss = loss.mean()
        elif self.args.use_batch_sum:
            batch_sum = torch.mean(
                torch.cat([rewards_chosen, rewards_rejected], dim=0)
            ).square()
            loss = loss.mean() + self.args.bsr_lambda * batch_sum
        elif self.args.center_rewards_coefficient is not None:
            loss += self.args.center_rewards_coefficient * (rewards_chosen + rewards_rejected).pow()
            loss = loss.mean()
        else:
            loss = loss.mean()
        ...
        with torch.no_grad():
            ...
            if self.args.log_every_reward:
                metrics["chosen_rewards"] =  wandb.Histogram(
                    self.accelerator.gather(rewards_chosen).detach().cpu().float().numpy()
                )
                metrics["rejected_rewards"] =  wandb.Histogram(
                    self.accelerator.gather(rewards_rejected).detach().cpu().float().numpy()
                )
            metrics["accuracy"] = self.accelerator.gather(accuracy.float()).mean().item()
            metrics["Batch Mean"] = self.accelerator.gather(batch_sum).mean().item()

            self.log(metrics)
            
            del metrics, margin, accuracy, batch_sum
